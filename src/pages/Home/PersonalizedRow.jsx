 ...
 */
async function buildPersonalisedFeed(watchlistItems, seedCount = 4) {
  // 1. Get the most recently watched items (stored locally)
  const cwItems = getContinueWatching().slice(0, seedCount);
...
export default function PersonalizedRow({ onSelect }) {
  ...
  // Fetch personalised feed when watchlist changes
  useEffect(() => {
    let cancelled = false;
    setLoading(true);

    buildPersonalisedFeed(watchlistItems).then(({ items: feed, seedTitles: titles }) => {
      if (cancelled) return;
      setItems(feed);
